// build.mjs — 「今日の灯」をGistから取得して埋め込む自動ビルド版
import { writeFileSync } from "node:fs";

// ====== 設定（必要ならあなたのGistに差し替えOK）======
const GIST = {
...
async function fetchText(url) {
  const res = await fetch(url, { cache: "no-store" }); // Node20はfetchが標準
...
let dailyMsg = "";
try {
  dailyMsg = await fetchText(GIST.daily);
} catch (e) {
  dailyMsg = "（今日の灯は準備中です。Gistの公開設定やURLを確認してください。）";
  console.error(e);
}
...
<main>
  ...
  <!-- ここに後で「宙の天気」「ランキング」など他Gist連携セクションを増やせます -->
</main>
...
console.log("index.html generated with daily message.");
